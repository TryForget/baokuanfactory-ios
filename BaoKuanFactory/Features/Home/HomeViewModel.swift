...
final class HomeViewModel: ObservableObject {
    ...
    func bindServices(membershipService: MembershipService, appSettings: AppSettings) {
    ...
    func generate() {
        ...
        guard let membershipService else {
            presentError("会员服务未准备好，请稍后再试")
            return
        }

        guard membershipService.canGenerateMore() else {
            presentError("免费次数已用完，开通会员后可继续生成")
            return
        }
        ...
        Task {
            do {
                ...
                if result.remainingQuota != nil || result.isPremium != nil {
                    membershipService.applyServerQuota(
                        remainingQuota: result.remainingQuota,
                        isPremium: result.isPremium
                    )
                } else {
                    membershipService.consumeGenerationIfNeeded()
                }

                if appSettings?.autoSaveHistory ?? true {
